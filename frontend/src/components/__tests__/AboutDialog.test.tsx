...
vi.mock('@/hooks/useProfileContext', () => ({
  useProfileContext: () => ({
    profile: {
      github: 'https://github.com/FlySword6',
    },
  }),
}));
...
describe('AboutDialog', () => {
  ...
  it('renders GitHub link', () => {
    render(<AboutDialog open={true} onOpenChange={() => {}} />);
    const link = screen.getByText('朱健科的 GitHub');
    expect(link.closest('a')).toHaveAttribute(
      'href',
      'https://github.com/FlySword6',
    );
  });

  it('does not render original project link', () => {
    render(<AboutDialog open={true} onOpenChange={() => {}} />);
    expect(
  ...
  it('does not render when closed', () => {
    ...
    expect(screen.queryByText('关于这份 AI 简历')).not.toBeInTheDocument();
  });
});
